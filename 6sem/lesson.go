...
func main() {
	...
	for {
		...
		for{
			_, _ = fmt.Scan(&answer)
			b.WriteString(fmt.Sprintf("Введеннок число: %d \n", answer))
			if answer<1 || answer>100 {
				fmt.Println("Введенное число должно быть в диапазоне от 1 до 100.")
				b.WriteString("Введенное число должно быть в диапазоне от 1 до 100. \n")
			} else {
				break
			}
		}
